...
void DependencyGraph::refinalize() {
    ...
    // Find all non-textured entities and mark them as ready.
    for (auto pair : mMaterialToEntity) {
    ...
    // Find all ready textures and check the readiness of their corresponding materials.
    for (auto& pair : mTextureNodes) {
        if (pair.second->ready) {
            markAsReady(pair.first);
        }
    }
}
